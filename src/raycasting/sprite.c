...
/*   Updated: 2026/01/04 22:00:00 by yelu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    draw_cat(t_data *data)
{
	...
	while (++y < cur->height)
	{
		...
		while (++x < cur->width)
		{
			if ((int)cur->x + x >= 0 && (int)cur->x + x < WIDTH
				&& (int)cur->y + y >= 0 && (int)cur->y + y < HEIGHT)
			{
				color = get_pixel_color(cur, x, y);
				if ((color & 0x00FFFFFF) != 0)
					my_mlx_pixel_put(data, (int)cur->x + x, (int)cur->y + y, color);
			}
		}
	}
}
