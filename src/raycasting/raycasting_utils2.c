...
/*   Updated: 2026/01/03 00:55:56 by yelu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->ray.wall_x = data->player.pos_x + data->ray.perp_dist
		* data->ray.ray_dir_x;
	if (!side)
		data->ray.wall_x = data->player.pos_y + data->ray.perp_dist
			* data->ray.ray_dir_y;
	...
	data->ray.tex_num = (int)(data->ray.wall_x
			* (double)(data->tex[data->ray.side].img_width));
	if ((!side && data->ray.ray_dir_x > 0) || (side && data->ray.ray_dir_y < 0))
		data->ray.tex_num = data->tex[data->ray.side].img_width
			- data->ray.tex_num - 1;
}
...
void	ray_tex_wall(t_data *data, int x, int y)
{
	...
	color = *((unsigned int *)(data->tex[data->ray.side].addr
				+ (tex_y * data->tex[data->ray.side].line_len
					+ data->ray.tex_num
					* (data->tex[data->ray.side].bits_per_pixel / 8))));
